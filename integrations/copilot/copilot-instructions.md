...
Use the generated `.codeprism/slices/*.md` file as the first source for project structure, important files, and symbol locations. Slices are capped to about 8K estimated tokens by default; budgets above 16K require `--allow-large-context`. If a slice says it was capped, narrow with `codeprism query`, `codeprism references NODE_ID`, `codeprism get NODE_ID`, or `codeprism read PATH --mode signatures/diff` instead of raising the budget. Run `codeprism gain` when you need the estimated savings/freshness report. Run `codeprism watch . --once` when you want to refresh only if the current map is stale. For a whole file, prefer `codeprism read PATH --mode signatures` or `--mode diff` before opening the full source. If a context command warns that the map is stale, use `--refresh`; use `--strict-fresh` when stale context should fail. Verify in raw files before changing code.
