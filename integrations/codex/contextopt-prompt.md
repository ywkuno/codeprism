...
Use `codeprism prime "<task>" --changed` before broad exploration, then read the generated slice first. Keep the query narrow; slices are capped to about 8K estimated tokens by default, and budgets above 16K fail unless `--allow-large-context` is supplied. High `--limit` / `--max-tokens` values can erase the savings. If the slice was capped, use `codeprism query`, `codeprism get`, `codeprism references`, or `codeprism read --mode signatures/diff` instead of dumping more context into chat. If a conversation has compacted, do not rerun a broad prime; report current state and continue with targeted reads. If a context command warns that the map is stale, use `--refresh`; use `codeprism watch . --once` when you want to refresh only if needed; use `--strict-fresh` when stale context should fail. Map refreshes use a local `context.lock` and exit cleanly if another agent is mapping. CodePrism appends local command events to `.codeprism/live-trace.jsonl`; `codeprism visualize --outdir .codeprism/visual` can replay that trace as a lightweight inspection layer.
