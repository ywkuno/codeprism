...
Use `codeprism prime "<task>" --changed` before broad exploration, then read the generated slice first. If a context command warns that the map is stale, use `--refresh`; use `codeprism watch . --once` when you want to refresh only if needed; use `--strict-fresh` when stale context should fail. Map refreshes use a local `context.lock` and exit cleanly if another agent is mapping. CodePrism appends local command events to `.codeprism/live-trace.jsonl`; `codeprism visualize --outdir .codeprism/visual` can replay that trace as a lightweight inspection layer.
