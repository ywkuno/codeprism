...
- MVP3: Pixel Brain turns Live Trace and activity events into animated replay with timeline controls, speed control, event list, lightweight marker movement, agent trails, and token counters.
...
### 2. Activity stream
JSONL event stream generated by CodePrism or the agent runtime.
Examples:
- prime
- map
- query
...
- load graph JSON
- load normalized activity JSON or `.codeprism/live-trace.jsonl`
...
- keep the default effect lightweight: one active pulse, short dashed beams, and no background animation when paused
